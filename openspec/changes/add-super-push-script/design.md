...
Display the repository, fixed ref, full old/new SHAs, fast-forward result, commit list, raw diff stat, changed-file names, and the fact that local hooks are disabled. Require the case-sensitive phrase `Approved` from an interactive, non-redirected terminal, except for a fail-closed docs-only classifier based on Git raw and numstat metadata. It accepts only ordinary text files under `docs/` or `openspec/` and root README, CHANGELOG, CONTRIBUTING, SECURITY, or LICENSE variants; it rejects `AGENTS.md` at any depth, control-character or mixed paths, binary files, symlinks, submodules, malformed output, ambiguous output, and entries Git reports as renames or copies. This skips only the cmdlet confirmation: Codex still requires fresh native approval for the fixed broker and every push.
