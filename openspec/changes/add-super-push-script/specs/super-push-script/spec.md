...
### Requirement: Canonical standalone ownership
RickScripts SHALL own Super Push behavior and focused behavioral tests in one standalone PowerShell script outside the module export surface. The dependent dotfiles integration SHALL contain only the fixed no-argument security broker and Codex hook, policy, and install wiring; it SHALL execute the fixed RickScripts script in a no-profile PowerShell child and SHALL NOT copy implementation behavior.

#### Scenario: RickScripts artifact is installed
- **WHEN** this change is applied
- **THEN** one unexported RickScripts script contains the complete implementation and focused tests exercise that artifact
...
- **THEN** the broker invokes the fixed RickScripts artifact without duplicating its behavior

### Requirement: Exceptional path preserves ordinary protection
The default GitHub workflow SHALL remain feature branch plus pull request. Ordinary human, administrator, and Codex credentials SHALL receive no bypass capability; only the dedicated GitHub App MAY be an Always-allow ruleset bypass actor for explicitly selected Crisp repositories. Humans and Codex SHALL enter through `llm-super-push`; direct Codex invocation of the RickScripts script SHALL NOT be approved.
...
- **THEN** the script derives its canonical repository and fixed HTTPS push URL without caller input
...
- **THEN** the script fails before credential access or remote mutation

### Requirement: Exact fast-forward evidence and confirmation
The script SHALL resolve local `HEAD` and fetched remote main to full commit SHAs, reject a no-op or non-fast-forward candidate, and display the exact repository, fixed ref, old/new full SHAs, ancestry, commits, raw diff stat, raw full diff, and disabled local-hook behavior. It SHALL require one case-sensitive exact interactive confirmation containing the repository, new SHA, and fixed ref, with no unattended or bypass mode.
...
- **THEN** the script fails without minting a token or pushing

### Requirement: Repeated immediate state verification
The script SHALL perform complete fetch, identity, cleanliness, and fast-forward reads before confirmation, again immediately after confirmation and before credential access, and again after token minting immediately before the push. Each later read SHALL exactly match the confirmed repository, root, origin, target, old SHA, and new SHA; any drift SHALL abort and require a fresh invocation.
...
- **THEN** the script aborts before pushing and does not retry

#### Scenario: State remains exact
- **WHEN** both repeated reads match the confirmed state
- **THEN** the script may perform the one exact non-force push

### Requirement: Human-gated credential isolation
The App client ID and private key SHALL be read only within the bounded broker-started script process through fixed Crisp human account `2KC5FVMXXJGKDG7LGHWF2OJ2N4` and a human-only item outside the `Automation` vault and every Local Automation service-account scope. Credentials SHALL NOT reach a parent or general shell, disk, output, logs, command arguments, or tests. Codex use SHALL require fresh native task-scoped approval naming the repository, SHA, and mutating push.

#### Scenario: Human terminal use
- **WHEN** a human deliberately enters through `llm-super-push`
...
- **THEN** it may run only the exact no-argument broker with a TTY after fresh native approval and cannot read the App item directly
...
- **THEN** the script fails before Git mutation

### Requirement: Exactly one isolated non-force push
The only remote Git mutation SHALL be one push of the verified full SHA to `refs/heads/main` at the fixed HTTPS repository URL. The script SHALL NOT force, rewrite, retry, loop, change repository or ref, or use a fallback credential. It SHALL disable local hooks and prevent credential helpers, URL rewrites, trace output, ambient configuration, transport redirects, or prompts from receiving or leaking the installation token.

#### Scenario: Push succeeds
- **WHEN** GitHub accepts the exact fast-forward update through the App actor
- **THEN** the script reports the immutable update once and performs no other push

#### Scenario: Push is rejected
- **WHEN** GitHub rejects the update or a race makes it non-fast-forward
- **THEN** the script reports sanitized failure and does not retry or weaken protection

### Requirement: Credential disposal and sanitized audit evidence
The script SHALL attempt to revoke every minted installation token in cleanup, clear credential and Git authentication state, and never persist secret material. It SHALL report repository, fixed ref, old/new SHAs, installation identity, timestamp, push outcome, and revocation outcome without secrets. Revocation failure SHALL fail the command and report bounded server expiry; GitHub's App actor and pushed SHA SHALL remain authoritative audit evidence.
