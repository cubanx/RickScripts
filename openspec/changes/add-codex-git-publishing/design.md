...
- When every status entry is under exactly one `openspec/changes/<change-name>/` directory, publishing uses `<repo-slug>/<change-name>`, `docs: add <space-separated-change-name-with-leading-add-removed> OpenSpec`, the matching `Add ... OpenSpec` human title, and the proposal's `Why`, `What Changes`, and `Impact` sections as the PR body. This path does not invoke Codex. Multiple OpenSpec changes or any non-OpenSpec path use the normal Codex fallback.
