...
Describe 'Publish-GitChanges' {
    ...
    It 'uses deterministic metadata for a single OpenSpec change on the default branch' {
        ...
        $script:LastBody | Should -Match 'The proposal defines the intended daily dash renewals behavior\.'
        $script:LastBody | Should -Not -Match 'specification-only|No runtime behavior changes|strict validation'
    }
