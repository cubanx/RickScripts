BeforeAll {
    ...
    function gh {
        ...
        if ($command -like 'api graphql *') {
            $number = [int]($Arguments[-1] -replace '^number=', '')
            $data = @{ dw = $null; ia = $null; ea = $null; rs = $null }
            foreach ($repository in @($script:LookupByNumber[$number])) {
                $data[$repository] = @{ pullRequest = @{ isDraft = $false; state = 'OPEN' } }
            }
            return @{ data = $data } | ConvertTo-Json -Depth 4
        }
        ...
        if ($command -in @('pr view 61 --json headRefOid,title,url,statusCheckRollup', 'pr view https://github.com/Crisp-Inc/data-warehouse/pull/61 --json headRefOid,title,url,statusCheckRollup')) {
            $script:ViewCount++
            $head = if ($script:Scenario -eq 'changed-head' -and $script:ViewCount -gt 1) { '62abcdef' } else { '61abcdef' }
            $checks = @(
            ...
            return @{ headRefOid = $head; title = 'Publish Yoda Gong transcript consumer'; url = 'https://github.com/Crisp-Inc/data-warehouse/pull/61'; statusCheckRollup = $checks } | ConvertTo-Json -Depth 4
        }
        if ($command -eq 'pr merge 42 --auto --merge --match-head-commit 0123456789abcdef') {
        ...
        if ($command -eq 'pr merge 61 --merge --match-head-commit 62abcdef') { return }
...
Describe 'Watch-PullRequest' {
    BeforeEach {
        $script:RepositoryRoot = Join-Path ([System.IO.Path]::GetTempPath()) "rickscripts-wpr-repositories-$([guid]::NewGuid())"
        $script:PullRequestRepositories = @{}
        foreach ($token in @('dw', 'ia', 'ea', 'rs')) {
            $repository = Join-Path $script:RepositoryRoot $token
        ...
        $script:LookupByNumber = @{ 42 = @('ia'); 61 = @('dw') }
    ...
    It 'selects the sole configured repository containing a numeric pull request' {
    ...
    It 'rejects a numeric pull request found in multiple repositories' {
        $script:LookupByNumber[42] = @('ia', 'ea')

        { Watch-PullRequest 42 } | Should -Throw '*multiple configured repositories*'
        @($script:Calls | Where-Object { $_ -like 'pr view *' }).Count | Should -Be 0
    }

    It 'rejects a numeric pull request not found in a configured repository' {
        $script:LookupByNumber[42] = @()

        { Watch-PullRequest 42 } | Should -Throw '*No open non-draft pull request*'
        @($script:Calls | Where-Object { $_ -like 'pr view *' }).Count | Should -Be 0
    }

    It 'enables native auto-merge without polling or bypassing requirements' {
        Watch-PullRequest 42
    ...
    It 'dispatches Snowflake staging only for a data-warehouse pull request and waits for its check' {
        ...
        @($script:Calls | Where-Object { $_ -like 'pr merge 61 --auto *' }).Count | Should -Be 0
    }
    ...
    It 'surfaces refused auto-merge enablement' {
        $script:Scenario = 'failed-merge'
        { Watch-PullRequest 42 } | Should -Throw
    }

    It 'restarts data-warehouse checks when the head changes and merges only the replacement' {
        $script:Scenario = 'changed-head'
        $script:StagingComplete = $true

        Watch-PullRequest 61

        @($script:Calls | Where-Object { $_ -eq 'pr checks 61 --watch --fail-fast' }).Count | Should -Be 2
        $script:Calls | Should -Not -Contain 'pr merge 61 --merge --match-head-commit 61abcdef'
        $script:Calls | Should -Contain 'pr merge 61 --merge --match-head-commit 62abcdef'
